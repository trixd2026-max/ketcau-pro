...
export default function BeamPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    alert('Đã thêm dầm và tính toán!');
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
        <h2 className="font-semibold text-lg mb-4">Nhập liệu Dầm</h2>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
        <h2 className="font-semibold text-lg mb-4">Danh sách dầm</h2>
        {beams.length === 0 ? <p className="text-slate-500 text-sm">Chưa có dầm.</p> : (
          <div className="space-y-3">
            {beams.map(b => {
              const res = project?.results[b.id];
              return (
                <div key={b.id} className="border border-slate-200 dark:border-slate-700 rounded-lg p-4">
