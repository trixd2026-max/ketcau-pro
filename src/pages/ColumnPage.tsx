...
export default function ColumnPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      type: 'column' as const, sectionType: form.sectionType,
  ...
  const handleDelete = (id: string, name: string) => {
    ...
    removeElement(id); if (editingId === id) resetForm();
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-20 md:pb-0">
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="block text-sm font-medium mb-1">l0 (m)</label>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
        ...
        {columns.length === 0 ? <p className="text-slate-500 text-sm">Chưa có cột nào.</p> : (
                ...
                <div key={c.id} className={`border rounded-lg p-4 ${editingId === c.id ? 'border-blue-400 bg-blue-50/50 dark:bg-blue-900/20' : 'border-slate-200 dark:border-slate-700'}`}>
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <button onClick={() => loadToForm(c)} className="p-1.5 rounded hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500" title="Sửa"><Pencil size={14} /></button>
                      <button onClick={() => { if (duplicateElement(c.id)) addToast('Đã nhân bản cột', 'success'); }} className="p-1.5 rounded hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500" title="Nhân bản"><Copy size={14} /></button>
                      <button onClick={() => handleDelete(c.id, c.name)} className="p-1.5 rounded hover:bg-red-50 dark:hover:bg-red-900/30 text-red-500" title="Xóa"><Trash2 size={14} /></button>
                    </div>
                  </div>
                  {res && (
                    <div className="mt-3">
                      <UtilizationBar value={res.utilization} />
                      <ul className="mt-2 text-xs text-slate-500 space-y-1">{res.details.slice(0, 3).map((d, i) => <li key={i}>• {d}</li>)}</ul>
